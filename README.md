...
v0.1
## Descrição
Este sistema consiste em um ambiente simulado baseado no clássico _Pacman_, onde o agente principal (_Pacman_) é controlado de forma autónoma por um algoritmo de busca $A*$. Utilizando a **Distância de Manhattan** como heurística adaptativa, o agente é capaz de calcular rotas otimizadas para recolher elementos do cenário enquanto desvia dinamicamente de fantasmas através da manipulação de pesos e custos na matriz do mapa.
## Funcionalidades
...
Naturalmente, existem diversas formas de implementar um agente que controle o personagem. Pensei inicialmente em utilizar _**Q-Learning**_ para experimentar o uso de *machine learning*, mas devido ao prazo de entrega e requisitos passados pela professora, optei por um algoritmo mais simples, que permitisse a adição de um *player* autônomo mantendo uma relativa simplicidade.
### O que controla os fantasmas?
Em conformidade com o jogo original, criado por **Tōru Iwatani**, os 4 fantasmas presentes no mapa (**Blinky**, **Pinky**, **Inky** and **Clyde**) possuem diferentes 'personalidades', ou seja, um algoritmo ligeiramente diferente. A ideia é que por um determinado período de tempo cada fantasma possua um *target* para onde devem se locomover, a entidade (fantasma) tenta então mover-se para uma direção (cima, baixo, direita, esquerda). Imediatamente, a direção da qual ele veio é eliminada, assim como qualquer direção que leve a uma parede. Para decidir entre as opções restantes, eles utilizam a fórmula da **Distância de Manhattan**, descrita na *[Wikipedia](https://pt.wikipedia.org/wiki/Geometria_do_t%C3%A1xi)* como:
...
-  ***Chase***: O fantasma vira em 180 graus (o que resulta nele virando para trás, uma exceção à regra citada anteriormente) e seu *target* é atualizado para a posição do jogador **a cada tomada de decisão**.
...
Não, eu escrevi mas seguindo este tutorial https://youtu.be/9H27CimgPsQ?si=CkwaHQNqF-9Yocta, também utilizando este vídeo como apoio https://youtu.be/ataGotQ7ir8?si=tMRHgmOcDYblITO.
### Quais as entradas e saídas da aplicação?
- **Entrada**: Jogador escolhe se vai controlar o personagem, ou se deixará o algoritmo controla-lo.
...
***Python*** e ***Pygame***, assim como ***Git*** e ***Github*** para gerenciamento de repositório.
## Referências
